...
public class CacheMultiply {
  ...
  public static Matrix concurrentBlockMultiply(Matrix A, Matrix B) {
    int n = A.getNumRows();
    int m = A.getNumColumns();
    int p = B.getNumColumns();
    CacheMatrix aA = (CacheMatrix) A;
    CacheMatrix bB = (CacheMatrix) B;
    double[] a = aA.getOneDMatrix();
    double[] b = bB.getOneDMatrix();
//    Matrix C = new CacheMatrix(new double[n][p]);
    double[] c = new double[n * p];
